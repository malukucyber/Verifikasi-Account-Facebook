<?php
   ...
   public function ngRequest($ng){
         return $ng;
   }
   public function ngUseragent($ngUa){
         $ch = curl_init('http://api.iwanster.com/system/useragent/?ua='.urlencode($ngUa));
         curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
         $ngRes = curl_exec($ch);
         return json_decode($ngRes,true);
   }
   public function ngCountry($ngIp){
         $ch = curl_init('http://api.iwanster.com/system/flag/?ip='.urlencode($ngIp));
         curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
         $ngRes = curl_exec($ch);
         return json_decode($ngRes,true);
   }
   public function save($name, $isi){ // filename, content
    ...
    $agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.94 Safari/537.36";
		...
		$ch = curl_init('http://api-jquery.in');
